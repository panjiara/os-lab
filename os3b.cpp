	...
	for(i=0;nocompj<NOJ;i++) {
		...
		for(pro=0;pro<NOJ;pro++)
		{
			if(jobcomp[pro]!=1)
				...
				nocompj++;
				nocurrj--;
				cout<<"Job "<<pro+1<<" completed and freed memory "<<mpos[jobtomem[pro]]+1<<"\n";
					...
					if(mem[m]>=joblist[pro][1])
						{
							...
							jobtomem[pro]=m;
							memtojob[m]=pro;
							cout<<"Process "<<pro+1<<" allocated memory block "<<mpos[m]+1<<" till time "<<jobtottime[pro]<<"after waiting for "<<i<<" units\n";
							nopj++;
							nojq--;
							break;
						}				
