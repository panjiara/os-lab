...
int main() {
	int i,j,pro,tno=0;
	int joblist[NOJ][2]={5,5760,4,4190,8,3290,2,2030,2,2550,6,6990,8,8940,10,740,7,3930,6,6890,5,6580,8,3820,9,9140,10,420,10,220,7,7540,3,3210,1,1380,9,9450,3,3610,7,7540,2,2710,8,8390,5,5950,10,760};
	...
	int jpos[NOJ];for(i=0;i<NOJ;i++) jpos[i]=i;
	int key,key1,key2;
	  for(j=1;j<NOJ;j++)
	  {
	     key=joblist[j][0];
	     ...
	     key2=jpos[j];
	     i=j-1;
	     while(joblist[i][1]>key1 && i>=0)
	     {
		 joblist[i+1][0]=joblist[i][0];
		 joblist[i+1][1]=joblist[i][1];
		 jpos[i+1]=jpos[i];
		 i--;
	     }
	     joblist[i+1][0]=key;
	  ...
	  int mpos[NOMB];for(i=0;i<NOMB;i++) mpos[i]=i;
	  for(j=1;j<NOMB;j++)
	  {
	     key=mem[j];
	     key2=mpos[j];
	...
	for(i=0;nocompj<NOJ;i++) {
		...
		for(pro=0;pro<NOJ;pro++)
		{
			if(jobcomp[pro]!=1)
				...
				nocompj++;
				nocurrj--;
				cout<<"Job "<<jpos[pro]+1<<" completed and freed memory "<<mpos[jobtomem[pro]]+1<<"\n";
					...
					if(mem[m]>=joblist[pro][1])
						{
							...
							jobtomem[pro]=m;
							memtojob[m]=pro;
							cout<<"Process "<<jpos[pro]+1<<" allocated memory block "<<mpos[m]+1<<" till time "<<jobtottime[pro]<<"after waiting for "<<i<<" units\n";
							nopj++;
							nojq--;
							break;
						}				
